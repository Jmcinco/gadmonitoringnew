...
<head>
    ...
    <title>Roles Management - GAD System</title>
    <!-- Bootstrap CSS -->
            ...
            <ul class="nav nav-pills flex-column nav-links">
                ...
                <li class="nav-item">
                    ...
                    <ul class="sub-menu">
                        ...
                        <li><a href="<?= site_url('/Secretariat/RolesManagement') ?>" class="active">Roles</a></li>
                    </ul>
                </li>
            </ul>
        </div>
    ...
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2" id="pageTitle">
                <i class="bi bi-person-badge text-primary"></i> Roles Management
            </h1>
        </div>
        <!-- Roles Table -->
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Roles Records</h5>
                        ...
                        <?php if (!empty($roles)): ?>
                            <?php $i=1; foreach($roles as $role): ?>
                                <tr>
                                    ...
                                    <td><?= esc($role['role']) ?></td>
                                    <td>
                                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editRoleModal<?= $role['role_id'] ?>">Edit</button>
                                        <form action="<?= site_url('Secretariat/Roles/delete/'.$role['role_id']) ?>" method="post" class="d-inline" onsubmit="return confirm('Delete this role?');">
                                            <?= csrf_field() ?>
                            ...
                            <?php endforeach ?>
                        <?php else: ?>
                            <tr><td colspan="3" class="text-center text-muted">No role records found.</td></tr>
                        <?php endif ?>
                    </tbody>
                </table>
            </div>
        ...
        <?php if (!empty($roles)): ?>
            <?php foreach($roles as $role): ?>
                <div class="modal fade" id="editRoleModal<?= $role['role_id'] ?>" tabindex="-1" aria-labelledby="editRoleModalLabel<?= $role['role_id'] ?>" aria-hidden="true">
                    <div class="modal-dialog">
                        <form class="modal-content" action="<?= site_url('Secretariat/Roles/update/'.$role['role_id']) ?>" method="post">
                            <?= csrf_field(); ?>
                            <div class="modal-header">
                                <h5 class="modal-title" id="editRoleModalLabel<?= $role['role_id'] ?>">Edit Role</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label">Role</label>
                                    <input type="text" class="form-control" name="role" value="<?= esc($role['role']) ?>" required>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="addRoleModal" tabindex="-1" aria-labelledby="addRoleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form class="modal-content" action="<?= site_url('Secretariat/Roles/store') ?>" method="post">
                    <?= csrf_field(); ?>
                    <div class="modal-header">
