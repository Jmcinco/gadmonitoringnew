...
<head>
    ...
    <title>Positions Management - GAD System</title>
    <!-- Bootstrap CSS -->
            ...
            <ul class="nav nav-pills flex-column nav-links">
                ...
                <li class="nav-item">
                    ...
                    <ul class="sub-menu">
                        ...
                        <li><a href="<?= site_url('/Secretariat/PositionsManagement') ?>" class="active"><i class="bi bi-briefcase me-2"></i>Positions</a></li>
    ...
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2" id="pageTitle">
                <i class="bi bi-person-badge text-primary"></i> Positions Management
            </h1>
        </div>
        <!-- Positions Table -->
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Positions Records</h5>
                    ...
                    <tbody>
                        <?php if (!empty($positions)): ?>
                            <?php $i=1; foreach($positions as $position): ?>
                                <tr>
                                    <td><?= $i++ ?></td>
                                    <td><?= esc($position['position']) ?></td>
                                    <td><?= esc($position['code']) ?></td>
                                    <td>
                                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editPositionModal<?= $position['pos_id'] ?>">Edit</button>
                                        <form action="<?= site_url('Secretariat/Positions/delete/'.$position['pos_id']) ?>" method="post" class="d-inline" onsubmit="return confirm('Delete this position?');">
                                            <?= csrf_field() ?>
                            ...
                            <?php endforeach ?>
                        <?php else: ?>
                            <tr><td colspan="4" class="text-center text-muted">No position records found.</td></tr>
                        <?php endif ?>
                    </tbody>
                </table>
            </div>
        ...
        <?php if (!empty($positions)): ?>
            <?php foreach($positions as $position): ?>
                <div class="modal fade" id="editPositionModal<?= $position['pos_id'] ?>" tabindex="-1" aria-labelledby="editPositionModalLabel<?= $position['pos_id'] ?>" aria-hidden="true">
                    <div class="modal-dialog">
                        <form class="modal-content" action="<?= site_url('Secretariat/Positions/update/'.$position['pos_id']) ?>" method="post">
                            <?= csrf_field(); ?>
                            <div class="modal-header">
                                <h5 class="modal-title" id="editPositionModalLabel<?= $position['pos_id'] ?>">Edit Position</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label">Position</label>
                                    <input type="text" class="form-control" name="position" value="<?= esc($position['position']) ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Code</label>
                                    <input type="text" class="form-control" name="code" value="<?= esc($position['code']) ?>" required>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="addPositionModal" tabindex="-1" aria-labelledby="addPositionModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form class="modal-content" action="<?= site_url('Secretariat/Positions/store') ?>" method="post">
                    <?= csrf_field(); ?>
                    <div class="modal-header">
